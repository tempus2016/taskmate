 ...
 * Apply point-awarding bonuses to children (e.g. "Tidied bedroom").
 * Parents can manage bonus definitions and tap to apply them instantly.
 *
 * Version: 0.0.1
 */
...
class TaskMateBonusesCard extends LitElement {
  ...
  static getConfigElement() {
    return document.createElement("taskmate-bonuses-card-editor");
  }
  ...
  async _applyBonus(bonus) {
    ...
    try {
      await this.hass.callService("taskmate", "apply_bonus", {
        bonus_id: bonus.id,
        child_id: child.id,
      });
      this._showToast(this._t('bonuses.toast_applied', { points: bonus.points, pointsName: this._getPointsName(), childName: child.name }));
      // Flash the row
      ...
      this._showToast(this._t('bonuses.toast_apply_failed'));
    } finally {
  ...
  async _saveEdit() {
    ...
    try {
      await this.hass.callService("taskmate", "update_bonus", {
        ...
        icon: this._editingBonus.icon || "mdi:star-circle-outline",
      });
      this._editingBonus = null;
    } catch (e) {
  ...
  async _deleteBonus(id) {
    try {
      await this.hass.callService("taskmate", "remove_bonus", { bonus_id: id });
    } catch (e) {
      this._showToast(this._t('bonuses.toast_delete_failed'));
    }
  }
    ...
    return html`
      <div class="bonus-row" data-bonus-id="${b.id}">
        <div class="points-badge">
          <ha-icon icon="${b.icon || "mdi:star-circle-outline"}"></ha-icon>
          <div class="points-value">+${b.points}</div>
        ...
        ` : html`
          <button class="apply-btn"
            ...
            ${isLoading
              ...
              : html`<ha-icon icon="mdi:plus-circle-outline"></ha-icon> ${this._t('common.apply')}`
            }
          </button>
        `}
      </div>
      ...
      <div class="edit-form">
        ...
        <div class="form-row">
          <div class="form-field">
            ...
            <input type="text" .value=${b.icon || "mdi:star-circle-outline"}
    ...
    return html`
      <div class="edit-form">
        <div class="form-row">
          <div class="form-field" style="flex:2">
            <label>${this._t('bonuses.form_name_label')}</label>
            <input type="text" placeholder="${this._t('bonuses.form_name_placeholder')}" .value=${f.name}
        ...
        <div class="form-actions">
          ...
          <button class="btn-save" @click=${this._saveNew}>${this._t('bonuses.add_bonus')}</button>
        </div>
      </div>
    `;
  }
      ...
      <ha-card>
        ...
        <div class="card-header">
          <div class="header-left">
            ...
            <span class="header-title">${this.config.title || this._t('bonuses.default_title')}</span>
          </div>
          <div class="header-actions">
            ${bonuses.length ? html`
              <span class="bonus-count">${bonuses.length}</span>
            ` : ""}
            <button class="icon-btn ${this._editMode ? "active" : ""}" title="${this._t('bonuses.manage_title')}"
        ...
        <div class="card-content">
          ${visible.length === 0 && !this._showNewForm ? html`
            <div class="empty-state">
              ...
              <div class="empty-title">${this._t('bonuses.empty_title')}</div>
              <div class="empty-sub">${this._t('bonuses.empty_sub')}</div>
            </div>
          ` : ""}
          ...
          ${child && !this._editMode ? html`
            <div style="text-align:center; font-size:0.8rem; color:var(--text-secondary); padding-top:4px;">
              ${this._t('bonuses.applying_to', { childName: child.name, points: child.points, pointsName: this._getPointsName() })}
            </div>
          ` : ""}
        </div>
      </ha-card>
  ...
  _renderDesigned(design) {
    ...
    const rows = design === "playroom"
      ? html`<div class="d-pl-list">${visible.map(b => this._renderDesignedRow(b, design))}</div>`
      : html`${visible.map(b => this._renderDesignedRow(b, design))}`;
