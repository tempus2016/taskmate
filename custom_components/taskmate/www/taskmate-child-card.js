...
class TaskMateChildCard extends LitElement {
   ...
   *
   * Delegates to the shared engine in taskmate-sounds.js — this card used to
   * carry its own copy of every synthesised sound, which drifted out of sync
   * with the other copy in taskmate-config-sounds.js.
   *
   * @param {string} soundName - built-in name, or "custom:<file>" (#856)
   */
  _playSound(soundName) {
    const engine = window.__taskmate_sounds;
    if (!engine) return;
    engine.play(soundName, this._customSounds());
  }

  /** Uploaded custom sounds (#856). Lives on the chores sensor; the attr
   *  resolver merges it in, so the card reads it off the primary entity. */
  _customSounds() {
    const attrs = (window.__taskmate_attrs && window.__taskmate_attrs(this.hass, this.config?.entity))
      || this.hass?.states?.[this.config?.entity]?.attributes || {};
    return attrs.custom_sounds || [];
  }
