...
def _coord(due=(), settings=None, tts_entities=()):
    ...
    coord.hass.states.async_all = MagicMock(
        return_value=[MagicMock(entity_id=e) for e in tts_entities])
    return coord
...
class TestMessage:
    ...
    def test_blank_template_falls_back_to_the_default(self):
        ...
        assert coord.build_read_aloud_message("kid1") == DEFAULT_DONE_TEMPLATE.format(
            name="Ella", count=0, chores="")
    ...
    def test_bad_placeholder_does_not_silence_the_feature(self):
        ...
        assert coord.build_read_aloud_message("kid1") == DEFAULT_TEMPLATE.format(
            name="Ella", count=2, chores="a and b")
...
class TestSpeaking:
    ...
    async def test_explicit_arguments_win_over_settings(self):
        coord = _coord([_chore("a")],
                       {"read_aloud_media_player": "media_player.kitchen",
                        "read_aloud_tts_entity": "tts.configured"},
                       tts_entities=["tts.discovered"])
        await coord.async_read_aloud("kid1", media_player="media_player.bedroom",
