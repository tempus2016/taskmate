...
def test_under_cap_ok():
    ...
    coord = _coord(
        {"spend_cap_enabled": True, "spend_cap_period": "weekly", "spend_cap_amount": "100"}, REWARDS, claims
    )
    with (
...
def test_over_cap_raises():
    ...
    coord = _coord({"spend_cap_enabled": True, "spend_cap_period": "weekly", "spend_cap_amount": "60"}, REWARDS, claims)
    with (
        patch("homeassistant.util.dt.now", return_value=now),
        patch("homeassistant.util.dt.as_local", side_effect=lambda d: d),
    ):
